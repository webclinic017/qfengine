...
from qfengine.execution.order import Order
...
class PortfolioConstructionModel(object):
    ...
    def __call__(self, dt, stats=None):
        """
            ...
            if self.optimiser:
                weights = self.optimiser(dt, initial_weights=weights)
        ...
        # create a null weight vector (zero for all Assets).
        if self.alpha_model:
            alpha_weights = self.alpha_model(dt)
        else:
            alpha_weights = self._create_zero_target_weights_vector(dt)

        # If a risk model is present use it to obtain risk factors matrix
        if self.risk_model:
            risk_factors_matrix = self.risk_model(dt, alpha_weights = alpha_weights)
        else:
            risk_factors_matrix = None

        # Run the portfolio optimization with alpha_weights & risk_factors_matrix
        if self.optimiser:
            weights = self.optimiser(alpha_weights = alpha_weights,
