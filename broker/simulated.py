...
from qfengine.portfolio.portfolio import Portfolio
from qfengine.broker.broker import Broker
from qfengine.broker.transaction.transaction import Transaction
from qfengine.broker.fee_model.fee_model import FeeModel
...
class SimulatedBroker(Broker):
    ...
    def __init__(
        ...
        if settings.PRINT_EVENTS:
            print('Initializing simulated broker "%s"...' % self.account_id)
    ...
    def _set_base_currency(self, base_currency):
        ...
        currency is currently not supported by qfengine.
        Parameters
        ----------
        base_currency : `str`
        ...
        if base_currency not in settings.SUPPORTED['CURRENCIES']:
            raise ValueError(
                "Currency '%s' is not supported by qfengine. Could not "
    ...
    def _set_fee_model(self, fee_model):
        ...
        if issubclass(fee_model.__class__, FeeModel):
            ...
            raise TypeError(
                ...
                "FeeModel subclass. Failed to create the "
    ...
    def update(self, dt):
        ...
        if self.exchange.is_open_at_datetime(self.current_dt):
            ...
            sorted_orders = sorted(orders, key=lambda x: x[1].direction) # sell first then buy (neg to pos)
